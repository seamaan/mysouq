...
<?php
        //echo 'public/uploads/'.$folder->name.'/thumb<br>';
        $path=base_path('public/uploads/'.$folder->name.'/thumb');
        $name=$folder->name;
        $files=File::allFiles($path);
...
//        $path=$filename;
        ...
        //$albumid=\App\Album::where('name',$name)->first()->id;
        ?>
            ...
            <img src="{{url('/public/uploads/'.$path)}}" class="thumbnail " style="width: 100%;float: right">
        </label>
...
<script type="text/javascript">
    ...
    function deletsetlect() {
        ...
        $.ajax({
            ...
            dataType:'json',
            data: {'filename': checkedValue,_token:token},
            beforeSend:function () {
